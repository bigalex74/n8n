...
ORIGINALS_FOLDER_PATH = os.getenv("ORIGINALS_FOLDER_PATH", "/Yulia/+ Test/выгружено")
GLOSSARY_FOLDER_PATH = os.getenv("GLOSSARY_FOLDER_PATH", "/Yulia/+ Test/глоссарии")
...
class OriginalsMergeRequest(BaseModel):
    name: str = Field(min_length=1, max_length=180)
    overwrite: bool = False


class OriginalsFragmentsRequest(BaseModel):
    name: str = Field(min_length=1, max_length=180)
    fragments: List[str] = Field(default_factory=list, max_length=50)
    expected_sha256: Optional[str] = Field(default=None, max_length=64)


class OriginalsGarbageRequest(BaseModel):
    name: str = Field(min_length=1, max_length=180)
    start: str = Field(min_length=1, max_length=12_000)
    end: str = Field(min_length=1, max_length=12_000)
    expected_sha256: Optional[str] = Field(default=None, max_length=64)
...
ORIGINALS_SOURCE_RE = re.compile(r"^page-content\((\d+)\)\.txt$", re.IGNORECASE)


def originals_source_key(name: str):
    match = ORIGINALS_SOURCE_RE.fullmatch(name)
    return int(match.group(1)) if match else None


def validated_originals_name(name: str, extension: str) -> str:
    name = name.strip()
    ...
    if not name.casefold().endswith(extension.casefold()):
        name += extension
    if name.casefold().endswith(extension.casefold()) is False:
        raise HTTPException(status_code=400, detail="Некорректное расширение файла")
    return name


async def current_originals_listing() -> dict:
    files = []
    offset = 0
    total = None
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            while total is None or offset < total:
                response = await client.get(
                    f"{YANDEX_DISK_API_URL}/resources",
                    headers=yandex_auth_headers(),
                    params={"path": ORIGINALS_FOLDER_PATH, "limit": 1000, "offset": offset},
                )
                response.raise_for_status()
                payload = response.json()
                if payload.get("type") != "dir":
                    raise ValueError("Originals source is not a directory")
                embedded = payload.get("_embedded") or {}
                items = embedded.get("items") or []
                files.extend(item for item in items if item.get("type") == "file" and item.get("name") and item.get("path"))
                total = int(embedded.get("total", len(items)))
                if not items:
                    break
                offset += len(items)
    except (httpx.HTTPError, TypeError, ValueError, KeyError) as exc:
        raise HTTPException(status_code=503, detail="Папка исходных файлов на Яндекс Диске недоступна") from exc
    sources = sorted((item for item in files if originals_source_key(item["name"]) is not None), key=lambda item: originals_source_key(item["name"]))
    merged = sorted(
...
async def originals_download_text(item: dict) -> str:
    ...
    if len(content) > 20_000_000:
        raise HTTPException(status_code=413, detail="TXT слишком большой")
    try:
        return content.decode("utf-8-sig")
    except UnicodeDecodeError as exc:
        raise HTTPException(status_code=422, detail="TXT должен быть в кодировке UTF-8") from exc


def normalize_originals_newlines(text: str) -> str:
    """Make pasted browser text and files comparable without weakening exact matching."""
    return text.replace("\r\n", "\n").replace("\r", "\n")
...
def originals_remove_between(text: str, start: str, end: str) -> tuple[str, dict]:
    """Remove every complete start..end block, preserving incomplete tails safely."""
    source = normalize_originals_newlines(text)
    start = normalize_originals_newlines(start)
    end = normalize_originals_newlines(end)
    if not start or not end:
        raise HTTPException(status_code=400, detail="Заполните начало и конец мусорного блока")
    result = []
    cursor = 0
    removed_blocks = 0
    unclosed_starts = 0
    while True:
        start_at = source.find(start, cursor)
        if start_at < 0:
            result.append(source[cursor:])
            break
        end_at = source.find(end, start_at + len(start))
        if end_at < 0:
            # Do not delete an open-ended tail: its end marker may be missing.
            result.append(source[cursor:])
            unclosed_starts += 1
            break
        result.append(source[cursor:start_at])
        cursor = end_at + len(end)
        removed_blocks += 1
    return "".join(result), {
        "start_occurrences": source.count(start),
        "end_occurrences": source.count(end),
        "removed_blocks": removed_blocks,
        "unclosed_starts": unclosed_starts,
    }


def original_merged_item(listing: dict, name: str) -> dict:
    normalized = validated_originals_name(name, ".txt")
    item = next((candidate for candidate in listing["merged"] if candidate["name"].casefold() == normalized.casefold()), None)
    if not item:
...
GLOSSARY_GENDERS = {"", "мужской", "женский"}


def normalize_glossary_text(value: str) -> str:
    """Trim only outer whitespace/newlines while preserving the entered text inside."""
    return str(value or "").strip()


def validated_glossary_name(value: str) -> str:
    name = normalize_glossary_text(value)
    if not name:
        raise HTTPException(status_code=400, detail="Укажите название глоссария")
    if any(ord(char) < 32 for char in name) or any(char in name for char in "/\\"):
        raise HTTPException(status_code=400, detail="Название не должно содержать переносы строк, / или \\")
    return name


def glossary_filename(name: str) -> str:
    return f"glossary_{validated_glossary_name(name)}.xlsx"


def glossary_disk_path(name: str) -> str:
    return f"{GLOSSARY_FOLDER_PATH.rstrip('/')}/{glossary_filename(name)}"


def validated_glossary_file_name(filename: str) -> str:
    filename = str(filename or "").strip()
    if not filename or filename != Path(filename).name or not filename.casefold().endswith(".xlsx"):
        raise HTTPException(status_code=400, detail="Выберите файл XLSX из папки глоссариев")
    return filename


def glossary_name_from_filename(filename: str) -> str:
    stem = Path(validated_glossary_file_name(filename)).stem
    if stem.casefold().startswith("glossary_"):
        stem = stem[len("glossary_"):]
    return validated_glossary_name(stem)


async def current_glossary_folder_listing() -> List[dict]:
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            response = await client.get(
                f"{YANDEX_DISK_API_URL}/resources",
                headers=yandex_auth_headers(),
                params={
                    "path": GLOSSARY_FOLDER_PATH,
                    "limit": 1000,
                    "fields": "_embedded.items.name,_embedded.items.path,_embedded.items.type,_embedded.items.size,_embedded.items.modified",
                },
            )
            response.raise_for_status()
            items = (response.json().get("_embedded") or {}).get("items") or []
    except (httpx.HTTPError, TypeError, ValueError, KeyError) as exc:
        raise HTTPException(status_code=503, detail="Не удалось получить список глоссариев") from exc
    files = [
        {"name": item["name"], "size": item.get("size"), "modified": item.get("modified")}
        for item in items
        if item.get("type") == "file" and str(item.get("name") or "").casefold().endswith(".xlsx")
    ]
    return sorted(files, key=lambda item: natural_file_name_key(item["name"]))


def glossary_entry_label(entry: dict) -> str:
    parts = [str(entry["ko"]), str(entry["ru"])]
    if entry.get("gender"):
        parts.append(str(entry["gender"]))
    return " — ".join(parts)


def glossary_xml_value(value: str) -> str:
    # OOXML uses XML 1.0: keep tabs/newlines but exclude forbidden controls.
    safe = "".join(char for char in str(value) if ord(char) >= 32 or char in "\t\n\r")
    return xml_escape(safe)


def build_glossary_xlsx(entries: List[dict]) -> bytes:
    """Build a real Office Open XML workbook without third-party dependencies."""
    rows = [{"ko": "KO", "ru": "RU", "gender": "Пол"}, *entries]
    xml_rows = []
    for row_number, row in enumerate(rows, start=1):
        cells = []
        for column, key in zip(("A", "B", "C"), ("ko", "ru", "gender")):
            value = glossary_xml_value(row.get(key) or "")
            cells.append(
                f'<c r="{column}{row_number}" s="1" t="inlineStr">'
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
 <cols><col min="1" max="2" width="32" customWidth="1"/><col min="3" max="3" width="14" customWidth="1"/></cols>
...
<styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
 ...
 <cellXfs count="2"><xf numFmtId="0" fontId="0" fillId="0" borderId="0" xfId="0"/><xf numFmtId="0" fontId="0" fillId="0" borderId="1" xfId="0" applyBorder="1" applyAlignment="1"><alignment vertical="top" wrapText="1"/></xf></cellXfs>
...
<workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets><sheet name="Глоссарий" sheetId="1" r:id="rId1"/></sheets></workbook>''')
...
def parse_glossary_xlsx(content: bytes) -> List[dict]:
    """Read KO/RU/Пол from the first worksheet of a normal XLSX workbook."""
    ...
    try:
        with zipfile.ZipFile(io.BytesIO(content)) as archive:
            ...
            if "xl/sharedStrings.xml" in archive.namelist():
                ...
                shared = ["".join(node.itertext()) for node in shared_root.findall("s:si", namespace)]
            workbook = ET.fromstring(archive.read("xl/workbook.xml"))
            sheet = workbook.find(".//s:sheet", namespace)
            if sheet is None:
                raise ValueError("no worksheet")
            rel_id = sheet.attrib.get("{http://schemas.openxmlformats.org/officeDocument/2006/relationships}id")
            rels = ET.fromstring(archive.read("xl/_rels/workbook.xml.rels"))
            target = next(
                rel.attrib["Target"] for rel in rels
    ...
    for row in root.findall(".//s:sheetData/s:row", namespace):
        ...
        for cell in row.findall("s:c", namespace):
            ...
            if cell_type == "inlineStr":
                value = "".join(cell.find("s:is", namespace).itertext()) if cell.find("s:is", namespace) is not None else ""
            else:
    ...
    for row_number, (ko, ru, gender) in enumerate(parsed_rows[header_index + 1:], start=header_index + 2):
        ...
        if not ko and not ru and not gender:
            continue
        if not ko or not ru:
            raise HTTPException(status_code=400, detail=f"Строка {row_number}: заполните KO и RU")
        if gender not in GLOSSARY_GENDERS:
            raise HTTPException(status_code=400, detail=f"Строка {row_number}: недопустимое значение пола")
        entries.append({"ko": ko, "ru": ru, "gender": gender})
    return entries


def ensure_glossary_schema() -> None:
    conn = get_conn_pg()
    try:
        cur = conn.cursor()
        try:
            cur.execute(
                """
                CREATE TABLE IF NOT EXISTS glossary_workspaces (
                ...
                CREATE TABLE IF NOT EXISTS glossary_entries (
                    ...
                    owner_id bigint NOT NULL REFERENCES glossary_workspaces(owner_id) ON DELETE CASCADE,
                    ko text NOT NULL,
                    ru text NOT NULL,
                    gender text NOT NULL DEFAULT '' CHECK (gender IN ('', 'мужской', 'женский')),
                ...
                CREATE INDEX IF NOT EXISTS glossary_entries_owner_ru_idx ON glossary_entries(owner_id, ru);
                CREATE INDEX IF NOT EXISTS glossary_entries_owner_position_idx
                    ON glossary_entries(owner_id, position, id);
                """
            )
            conn.commit()
        finally:
...
def ensure_glossary_workspace(owner_id: int) -> None:
    ...
    try:
        ...
        try:
            cur.execute(
                "INSERT INTO glossary_workspaces (owner_id) VALUES (%s) ON CONFLICT (owner_id) DO NOTHING",
                (owner_id,),
            )
            conn.commit()
        finally:
...
def glossary_state(owner_id: int) -> dict:
    ...
    try:
        ...
        try:
            ...
            cur.execute(
                """
                SELECT id, ko, ru, gender, active, position
    ...
    for entry in active:
        ...
        while changed:
            ...
            ko_values = {item["ko"] for item in group}
            ru_values = {item["ru"] for item in group}
            for candidate in active:
                if candidate["id"] not in group_ids and (candidate["ko"] in ko_values or candidate["ru"] in ru_values):
                    group_ids.add(candidate["id"])
                    changed = True
        if len(group_ids) > 1:
            conflicts[entry["id"]] = [
                {key: candidate[key] for key in ("id", "ko", "ru", "gender")}
                for candidate in active if candidate["id"] in group_ids
            ]
    for row in rows:
        row.pop("position", None)
        row["conflict"] = row["id"] in conflicts
        row["conflict_variants"] = conflicts.get(row["id"], [])
    return {
...
def glossary_duplicate(owner_id: int, ko: str, ru: str, exclude_id: Optional[int] = None) -> Optional[tuple[str, dict]]:
    conn = get_conn_pg()
    try:
        cur = conn.cursor(cursor_factory=RealDictCursor)
        try:
            query = "SELECT id, ko, ru, gender, active FROM glossary_entries WHERE owner_id = %s"
            params: list = [owner_id]
            if exclude_id is not None:
                query += " AND id <> %s"
                params.append(exclude_id)
            query += " AND ko = %s ORDER BY position, id LIMIT 1"
            cur.execute(query, (*params, ko))
            row = cur.fetchone()
            if row:
                return "KO", dict(row)
            query = "SELECT id, ko, ru, gender, active FROM glossary_entries WHERE owner_id = %s"
            ...
            if exclude_id is not None:
                query += " AND id <> %s"
                params.append(exclude_id)
            query += " AND ru = %s ORDER BY position, id LIMIT 1"
            cur.execute(query, (*params, ru))
            row = cur.fetchone()
            return ("RU", dict(row)) if row else None
        finally:
            cur.close()
    finally:
        conn.close()
...
def normalized_glossary_entry(payload: GlossaryEntryRequest) -> tuple[str, str, str]:
    ko = normalize_glossary_text(payload.ko)
    ru = normalize_glossary_text(payload.ru)
    gender = str(payload.gender or "").strip().casefold()
    if not ko:
        raise HTTPException(status_code=400, detail="Заполните поле KO")
    if not ru:
        raise HTTPException(status_code=400, detail="Заполните поле RU")
    if gender not in GLOSSARY_GENDERS:
        raise HTTPException(status_code=400, detail="Выберите допустимое значение пола")
    return ko, ru, gender
...
def originals_operator(request: Request) -> dict:
    user = verified_telegram_user(request)
    if not is_ocr_operator(user):
        raise HTTPException(status_code=403, detail="Только оператор может работать с исходными файлами")
    return user


def glossary_operator(request: Request) -> dict:
    user = verified_telegram_user(request)
    if not is_ocr_operator(user):
        raise HTTPException(status_code=403, detail="Только оператор может работать с глоссарием")
    return user


@app.get("/api/originals/files")
async def get_originals_files(request: Request):
    originals_operator(request)
    listing = await current_originals_listing()
    return {
...
@app.post("/api/originals/merge")
async def merge_originals_txt(request: Request, payload: OriginalsMergeRequest):
    originals_operator(request)
    output_name = validated_originals_name(payload.name, ".txt")
    listing = await current_originals_listing()
    if not listing["sources"]:
        raise HTTPException(status_code=409, detail="Исходные файлы не найдены")
    existing = next((item for item in listing["files"] if item["name"].casefold() == output_name.casefold()), None)
    if existing and not payload.overwrite:
        raise HTTPException(status_code=409, detail="Файл уже существует. Перезаписать?")
    texts = await asyncio.gather(*(originals_download_text(item) for item in listing["sources"]))
    content = "\n\n".join(texts).encode("utf-8")
    await yandex_upload_resource(f"{ORIGINALS_FOLDER_PATH}/{output_name}", content, "text/plain; charset=utf-8")
    return {"created": True, "name": output_name, "source_count": len(texts)}


@app.post("/api/originals/fragments/preview")
async def preview_originals_fragments(request: Request, payload: OriginalsFragmentsRequest):
    originals_operator(request)
    item = original_merged_item(await current_originals_listing(), payload.name)
    text = await originals_download_text(item)
    updated, count, fragments = originals_apply_fragments(text, payload.fragments)
    return {
        "name": item["name"], "fragment_count": len(payload.fragments), "matches": count,
        "fragments": fragments,
        "sha256": hashlib.sha256(text.encode("utf-8")).hexdigest(),
        "result_sha256": hashlib.sha256(updated.encode("utf-8")).hexdigest(),
    }


@app.post("/api/originals/fragments/apply")
async def apply_originals_fragments(request: Request, payload: OriginalsFragmentsRequest):
    originals_operator(request)
    item = original_merged_item(await current_originals_listing(), payload.name)
    text = await originals_download_text(item)
    sha256 = hashlib.sha256(text.encode("utf-8")).hexdigest()
    if not payload.expected_sha256 or payload.expected_sha256 != sha256:
        raise HTTPException(status_code=409, detail="Файл изменился. Повторите проверку совпадений")
    updated, count, fragments = originals_apply_fragments(text, payload.fragments)
    if not count:
        raise HTTPException(status_code=409, detail="Совпадения не найдены")
    await yandex_upload_resource(item["path"], updated.encode("utf-8"), "text/plain; charset=utf-8")
    return {"updated": True, "name": item["name"], "removed_count": count, "fragments": fragments}


@app.post("/api/originals/garbage/preview")
async def preview_originals_garbage(request: Request, payload: OriginalsGarbageRequest):
    originals_operator(request)
    item = original_merged_item(await current_originals_listing(), payload.name)
    text = await originals_download_text(item)
    updated, report = originals_remove_between(text, payload.start, payload.end)
    return {
        "name": item["name"], "sha256": hashlib.sha256(text.encode("utf-8")).hexdigest(),
        "result_sha256": hashlib.sha256(updated.encode("utf-8")).hexdigest(), **report,
    }


@app.post("/api/originals/garbage/apply")
async def apply_originals_garbage(request: Request, payload: OriginalsGarbageRequest):
    originals_operator(request)
    item = original_merged_item(await current_originals_listing(), payload.name)
    text = await originals_download_text(item)
    sha256 = hashlib.sha256(text.encode("utf-8")).hexdigest()
    if not payload.expected_sha256 or payload.expected_sha256 != sha256:
        raise HTTPException(status_code=409, detail="Файл изменился. Повторите проверку мусорных блоков")
    updated, report = originals_remove_between(text, payload.start, payload.end)
    if not report["removed_blocks"]:
        raise HTTPException(status_code=409, detail="Полных блоков между началом и концом не найдено")
    await yandex_upload_resource(item["path"], updated.encode("utf-8"), "text/plain; charset=utf-8")
    return {"updated": True, "name": item["name"], **report}


@app.post("/api/originals/convert")
async def convert_originals_docx(request: Request, payload: OriginalsConvertRequest):
    originals_operator(request)
    item = original_merged_item(await current_originals_listing(), payload.name)
    output_name = f"{Path(item['name']).stem}.docx"
    listing = await current_originals_listing()
    existing = next((candidate for candidate in listing["files"] if candidate["name"].casefold() == output_name.casefold()), None)
    if existing and not payload.overwrite:
        raise HTTPException(status_code=409, detail="Файл уже существует. Перезаписать?")
    text = await originals_download_text(item)
    ...
    await yandex_upload_resource(
        f"{ORIGINALS_FOLDER_PATH}/{output_name}", buffer.read(),
...
async def delete_originals_sources(request: Request, payload: OriginalsDeleteRequest):
    originals_operator(request)
    if payload.confirmation != "DELETE_ORIGINAL_SOURCE_TXT":
        raise HTTPException(status_code=400, detail="Удаление исходных файлов не подтверждено")
    listing = await current_originals_listing()
    if not listing["sources"]:
        raise HTTPException(status_code=409, detail="Исходные файлы не найдены")
    for item in listing["sources"]:
        await yandex_trash_resource(item["path"])
    return {"deleted_count": len(listing["sources"]), "moved_to_trash": True}


@app.get("/api/glossary")
async def get_glossary(request: Request):
    user = glossary_operator(request)
    return glossary_state(int(user["id"]))


@app.get("/api/glossary/files")
async def get_glossary_files(request: Request):
    glossary_operator(request)
    return {"files": await current_glossary_folder_listing()}


@app.post("/api/glossary/load")
async def load_glossary_file(request: Request, payload: GlossaryFileRequest):
    user = glossary_operator(request)
    owner_id = int(user["id"])
    filename = validated_glossary_file_name(payload.filename)
    files = await current_glossary_folder_listing()
    if filename not in {item["name"] for item in files}:
        raise HTTPException(status_code=404, detail="Файл не найден в папке глоссариев")
    try:
        content = await yandex_download_resource(f"{GLOSSARY_FOLDER_PATH.rstrip('/')}/{filename}")
    except httpx.HTTPError as exc:
        raise HTTPException(status_code=502, detail="Не удалось скачать выбранный XLSX") from exc
    entries = parse_glossary_xlsx(content)
    ensure_glossary_workspace(owner_id)
    conn = get_conn_pg()
    try:
        cur = conn.cursor()
        try:
            ...
            for position, entry in enumerate(entries, start=1):
                cur.execute(
                    """
                    INSERT INTO glossary_entries (owner_id, ko, ru, gender, position)
                    VALUES (%s, %s, %s, %s, %s)
                    """,
                    (owner_id, entry["ko"], entry["ru"], entry["gender"], position),
                )
            cur.execute(
                """
        ...
        conn.close()
    state = glossary_state(owner_id)
    return {
        "loaded": True,
        "filename": filename,
        "entry_count": len(entries),
        "conflict_count": sum(1 for entry in state["active"] if entry["conflict"]),
        "state": state,
    }


@app.post("/api/glossary/name")
async def save_glossary_name(request: Request, payload: GlossaryNameRequest):
    user = glossary_operator(request)
    owner_id = int(user["id"])
    ...
    try:
        ...
        try:
            cur.execute(
                "UPDATE glossary_workspaces SET name = %s, updated_at = now() WHERE owner_id = %s RETURNING source_filename",
                (name, owner_id),
            )
            workspace = cur.fetchone()
            conn.commit()
...
async def add_glossary_entry(request: Request, payload: GlossaryEntryRequest):
    ...
    owner_id = int(user["id"])
    ensure_glossary_workspace(owner_id)
    ko, ru, gender = normalized_glossary_entry(payload)
    duplicate = glossary_duplicate(owner_id, ko, ru)
    if duplicate:
        field, entry = duplicate
        raise HTTPException(status_code=409, detail=f"Такое значение {field} уже существует: {glossary_entry_label(entry)}")
    conn = get_conn_pg()
    try:
        cur = conn.cursor(cursor_factory=RealDictCursor)
        try:
            cur.execute(
                ...
                INSERT INTO glossary_entries (owner_id, ko, ru, gender, position)
                VALUES (%s, %s, %s, %s,
                    COALESCE((SELECT max(position) + 1 FROM glossary_entries WHERE owner_id = %s), 1))
                RETURNING id, ko, ru, gender, active
                """,
            ...
            conn.rollback()
            raise HTTPException(status_code=409, detail="Такая запись уже существует") from exc
        finally:
            cur.close()
    finally:
        conn.close()
    return {"added": True, "entry": row}


@app.put("/api/glossary/entries/{entry_id}")
async def update_glossary_entry(request: Request, entry_id: int, payload: GlossaryEntryRequest):
    user = glossary_operator(request)
    owner_id = int(user["id"])
    ensure_glossary_workspace(owner_id)
    ko, ru, gender = normalized_glossary_entry(payload)
    duplicate = glossary_duplicate(owner_id, ko, ru, exclude_id=entry_id)
    if duplicate:
        field, entry = duplicate
        raise HTTPException(status_code=409, detail=f"Такое значение {field} уже существует: {glossary_entry_label(entry)}")
    conn = get_conn_pg()
    try:
        ...
        try:
            cur.execute(
                ...
                SET ko = %s, ru = %s, gender = %s, updated_at = now()
                WHERE id = %s AND owner_id = %s AND active IS TRUE
                RETURNING id, ko, ru, gender, active
                """,
                (ko, ru, gender, entry_id, owner_id),
            )
            row = cur.fetchone()
            if not row:
...
def resolve_glossary_conflict(owner_id: int, entry_id: int, payload: GlossaryEntryRequest) -> dict:
    ko, ru, gender = normalized_glossary_entry(payload)
    state = glossary_state(owner_id)
    entry = next((item for item in state["active"] if item["id"] == entry_id), None)
    if not entry:
        raise HTTPException(status_code=404, detail="Активная запись не найдена")
    group_ids = {item["id"] for item in entry.get("conflict_variants") or []}
    if len(group_ids) < 2:
        raise HTTPException(status_code=409, detail="У записи больше нет повторений")
    conn = get_conn_pg()
    try:
        cur = conn.cursor(cursor_factory=RealDictCursor)
        try:
            cur.execute(
                """
                SELECT id, ko, ru, gender, active
                FROM glossary_entries
                WHERE owner_id = %s AND active IS TRUE AND NOT (id = ANY(%s))
                  AND (ko = %s OR ru = %s)
                ORDER BY position, id LIMIT 1
                """,
                (owner_id, list(group_ids), ko, ru),
            )
            collision = cur.fetchone()
            if collision:
                raise HTTPException(status_code=409, detail=f"Выбранный вариант пересекается с другой записью: {glossary_entry_label(collision)}")
            cur.execute(
                """
                UPDATE glossary_entries
                SET active = false, updated_at = now()
                WHERE owner_id = %s AND id = ANY(%s) AND id <> %s
                """,
                (owner_id, list(group_ids), entry_id),
            )
            ...
            cur.execute(
                ...
                UPDATE glossary_entries
                SET ko = %s, ru = %s, gender = %s, updated_at = now()
                WHERE owner_id = %s AND id = %s AND active IS TRUE
                RETURNING id, ko, ru, gender, active
                """,
        ...
        conn.close()
    return {"resolved": True, "entry": dict(row), "removed_count": removed_count}


@app.put("/api/glossary/entries/{entry_id}/resolve")
async def resolve_glossary_entry(request: Request, entry_id: int, payload: GlossaryEntryRequest):
    user = glossary_operator(request)
    return resolve_glossary_conflict(int(user["id"]), entry_id, payload)


def set_glossary_entry_active(owner_id: int, entry_id: int, active: bool) -> dict:
    ensure_glossary_workspace(owner_id)
    conn = get_conn_pg()
    try:
        cur = conn.cursor(cursor_factory=RealDictCursor)
        try:
            cur.execute(
                ...
                WHERE id = %s AND owner_id = %s AND active IS DISTINCT FROM %s
                RETURNING id, ko, ru, gender, active
                """,
                (active, entry_id, owner_id, active),
            )
            row = cur.fetchone()
            if not row:
...
@app.post("/api/glossary/entries/{entry_id}/remove")
async def remove_glossary_entry(request: Request, entry_id: int):
    user = glossary_operator(request)
    return {"removed": True, "entry": set_glossary_entry_active(int(user["id"]), entry_id, False)}


@app.post("/api/glossary/entries/{entry_id}/restore")
async def restore_glossary_entry(request: Request, entry_id: int):
    user = glossary_operator(request)
...
async def export_glossary(request: Request):
    user = glossary_operator(request)
    state = glossary_state(int(user["id"]))
    name = validated_glossary_name(state["name"])
    if not state["active"]:
        raise HTTPException(status_code=409, detail="Добавьте хотя бы одну активную запись")
    filename = state.get("source_filename") or glossary_filename(name)
    content = build_glossary_xlsx(state["active"])
    try:
        await yandex_upload_resource(
            f"{GLOSSARY_FOLDER_PATH.rstrip('/')}/{validated_glossary_file_name(filename)}",
            content,
    ...
    return Response(
        ...
        headers={
            "Content-Disposition": f'attachment; filename="glossary.xlsx"; filename*=UTF-8\'\'{quote(filename)}',
...
async def originals_page():
    with open("static/originals/index.html", "r", encoding="utf-8") as f: return f.read()

@app.get("/glossary", response_class=HTMLResponse)
async def glossary_page():
    with open("static/glossary/index.html", "r", encoding="utf-8") as f: return f.read()
