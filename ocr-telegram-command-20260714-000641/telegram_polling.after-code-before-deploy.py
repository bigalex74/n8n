...
OCR_COMMAND_RE = re.compile(r'^/ocr(?:@[A-Za-z0-9_]+)?\s*$', re.IGNORECASE)
...
def is_ocr_command(text: str) -> bool:
    """Return True only for the exact /ocr command (with optional bot suffix)."""
    return bool(OCR_COMMAND_RE.fullmatch((text or '').strip()))


def handle_ocr_command(message: dict) -> bool:
    """Validate and forward /ocr. Returns True when the message was handled."""
    text = (message.get("text") or "").strip()
    command_token = text.split(maxsplit=1)[0].lower() if text else ""
    if not (command_token == "/ocr" or command_token.startswith("/ocr@")):
        return False

    chat_id = int((message.get("chat") or {}).get("id") or 0)
    from_id = int((message.get("from") or {}).get("id") or 0)

    if chat_id != MY_CHAT_ID or from_id != MY_CHAT_ID:
        logger.warning("Rejected /ocr command from unauthorized user_id=%s", from_id)
        send_message(chat_id, "Эта команда доступна только владельцу бота.")
        return True

    if not is_ocr_command(text):
        send_message(chat_id, "Для запуска пакетного OCR отправьте команду /ocr")
        return True
    ...
    try:
        ...
        send_message(chat_id, "Не удалось запустить OCR. Попробуйте ещё раз через минуту.")
    return True
            ...
            if data.get("ok") and data.get("result"):
                for update in data["result"]:
                    ...
                    if message:
                        ...
                        elif handle_ocr_command(message):
                            logger.info("Handled OCR command locally")
                        else:
