...
class OcrApiTests(unittest.IsolatedAsyncioTestCase):
    def test_originals_accept_only_the_strict_source_template_and_sort_numerically(self):
        names = ["page-content(10).txt", "page-content(2).txt", "page-content(1).txt", "page-content(2)(1).txt"]
        sources = sorted((name for name in names if main.originals_source_key(name) is not None), key=main.originals_source_key)
        self.assertEqual(sources, ["page-content(1).txt", "page-content(2).txt", "page-content(10).txt"])

    def test_originals_remove_all_exact_fragments(self):
