...
- Secrets Management: Infisical is deployed at /home/user/infisical, accessible via https://secrets.bigalexn8n.ru. Project ID: 1d44cf0c-94b5-4e64-bccd-9c4da8843fec. All scripts in n8n-docker (start, run, restart) are integrated with 'infisical run' for dynamic secret injection.
- SECURITY TASK (REMINDER): On April 22, 2026, purge all sensitive data from /home/user/n8n-docker/.env and other local .env files, keeping only templates.
- The n8n project is located at /home/user/n8n-docker. It uses Docker Compose with services: db (Postgres 16), n8n (latest), and pgAdmin4. n8n is configured to use host networking. A local Xray/Hiddify proxy is running on the host at 127.0.0.1:10808 (HTTP/HTTPS) to bypass regional restrictions for Telegram API access. The main domain is bigalexn8n.ru, managed by a native Caddy instance on the host which reverse-proxies to n8n on port 5678. Custom database tables include document_jobs, telegram_messages, and telegram_send_message. Secrets (Telegram Token, DB Passwords) are managed via Infisical at secrets.bigalexn8n.ru. Use the n8n-expert skill for detailed workflows and database schema management.
